// Generated by Creer at 09:03PM on May 07, 2015 UTC, git hash: '3d2d5f08585852420c55724fccd252e71c025a37'
...
var Checker = Class(GeneratedChecker, {
	...
	move: function(player, x, y) {
		...
		if(this.owner !== player) {
			game.throwInvalidGameLogic(commandingPlayer, "tried to move a checker they didn't own");
		}
		...
		if(!this.kinged) { // then check if they are moving the right direction via dy when not kinged
			if((yOffset == 1 && dy < 1) || (yOffset == -1 && dy > -1)) {
				game.throwInvalidGameLogic(player, "moved in the wrong vertical direction");
			}
		}
		...
		if(Math.abs(dx) === 2 && Math.abs(dy) === 2) { // then it's jumping something
			...
			if(!jumped) {
				game.throwInvalidGameLogic(player, "tried to jump nothing");
			}
			else if(jumped.owner.id === this.owner.id) {
				game.throwInvalidGameLogic(player, "tried to jump own checker");
			}
		}
		else if(Math.abs(dx) === 1 && Math.abs(dy) === 1) { // then they are just moving 1 tile diagonally
			if(game.checkerMovedJumped) {
				game.throwInvalidGameLogic(player, "current checker must jump again, not move diagonally one tile");
			}
			// else valid as normal move
		}
		else {
			game.throwInvalidGameLogic(player, "can't move there");
		}

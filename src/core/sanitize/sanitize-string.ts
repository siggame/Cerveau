 ...
 * @returns Always returns a string.
 */
export function sanitizeString(s: unknown, allowError: false): string;
 ...
 * @returns Always returns a string.
 */
export function sanitizeString(s: unknown, allowError = true): string | Error {
    if (allowError && isObject(s)) {
        return new Error(
            `'${s}' is an Object and cannot be reasonably cast to a string`,
        );
    }

...
import { WorkerGameSessionData } from "~/core/server/worker";
...
export interface Args {
    ...
    WORKER_DATA?: Immutable<WorkerGameSessionData>;
}
...
const parserArgs: Array<[
    ...
    [
        ["--log"],
        {
            ...
            dest: "LOG_TO_FILES",
            help:
                "store all logged strings to text files in LOGS_DIR/console/",
        },
    ],
    ...
    [
        ["--no-auto-update"],
        {
            ...
            defaultValue: true,
            help:
                "the updater will not try to autoUpdate when updates are found",
        },
    ],
];
...
const args: Args = {} as Args; // we will set it so that it becomes a valid
...
for (const key of Object.keys(parsedArgs)) {
    ...
    ((args as unknown) as { [key: string]: unknown })[key] = value;
}
...
if (args.RUN_PROFILER) {
    try {
        // v8-profiler is an optional dependency, so not a normal import.
